...
void plotFinalSpectrum_pPb(TString spectrumFile, TString fPYTHIA, TString fPOWHEG, TString ppfile, TString systematicsDir, TString type, TString output, TString fileType, int minradius = 2, int maxradius = 5)
{
    Double_t textSize     = 0.03;
    Int_t regnum          = 6;
    double alphasval = 0.25;
    ...
    TFile *fOutput = new TFile(Form("%s/FinalResults_pPb8TeV.root",outputDirRootFile.Data()),"RECREATE");
    ...
    for(int radius = minradius; radius <= maxradius; radius++){
        ...
        if(radius != minradius){
            ...
            TGraphErrors *ratiopp        = (TGraphErrors*)rdirpp->Get(Form("ratioStat_R0%i",radius));
            TGraphErrors *ratioSyspp     = (TGraphErrors*)rdirpp->Get(Form("ratioSyst_R0%i",radius));

            ratiopp = FormatSpectrum(ratiopp,radius,false,false,false,minradius,maxradius);
            ratioSyspp = FormatSpectrum(ratioSyspp,radius,true,false,false,minradius,maxradius);
    ...
    dummySpectrum->GetYaxis()->SetRangeUser(3e-6,400);
    ...
    dummySpectrumUnscaled->GetYaxis()->SetRangeUser(3e-6,400);
    ...
    dummySpectrumLogX->GetYaxis()->SetRangeUser(3e-6,400);
    ...
    dummySpectrumLogXUnscaled->GetYaxis()->SetRangeUser(3e-6,400);
    ...
    drawLatexAdd("p#minusPb #sqrt{#it{s}_{NN}} = 8.16 TeV",0.93,0.9, textSize,kFALSE, kFALSE, true);
    ...
    drawLatexAdd("p#minusPb #sqrt{#it{s}_{NN}} = 8.16 TeV",0.93,0.93, textSize,kFALSE, kFALSE, true);
    ...
    drawLatexAdd("p#minusPb #sqrt{#it{s}_{NN}} = 8.16 TeV",0.17,0.88, textSize,kFALSE, kFALSE, false);
    ...
    for(int radius = minradius; radius <= maxradius; radius++){
            ...
            dummyHistLower = new TH2F(Form("dummyHistLower_R0%i", radius),Form("dummyHistLower_R0%i", radius), 1000, minPt,maxPt, 1000., 0.8,2.9);
            ...
            drawLatexAdd("p#minusPb #sqrt{#it{s}_{NN}} = 8.16 TeV",0.93,0.87, 0.85*textsizeLabelsWidth,kFALSE, kFALSE, true);
