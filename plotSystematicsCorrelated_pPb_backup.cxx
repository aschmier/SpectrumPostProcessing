...
void plotSystematicsCorrelated_pPb(TString fSysConfig, TString type, Int_t radius, TString fileType, TString out)
{
    ...
    outputDirRootFile         = "/home/austin/alice/SystematicsRootFile/pPb";
    ...
    systematics_names.insert(systematics_names.end(), {"allunfolding","qoverptshift","clusterizer","maxtrackpt","maxclustere","hadcorr","seedcell","tracking","triggerswap"});
    ...
    fitfunc.insert(fitfunc.end(), {"pol0","pol0","pol0","pol0","pol0","pol0","pol0","pol0","pol0"});
    ...
    for(int name=0; name<nSys; name++){
        systematicsFile = Form("input_txt_files/pPbFiles/%s_%s.txt", systematics_names.at(name).Data(), type.Data());
    ...
    for(int name=0; name<nSys; name++){
        ...
        sysAvg->GetXaxis()->SetRangeUser(minPt,maxPt);
                sysAvg->GetXaxis()->SetRangeUser(0,5);
