...
void plotSystematicsCorrelated_pPb(TString fSysConfig, TString type, Int_t radius, TString fileType, TString out)
{
    //////////////////////////// Define Variables //////////////////////////////
    ...
    //if(radius == 2){
        plot_names.insert(plot_names.end(), {"Unfolding","Embedding","Clusterizer","Max Track p_{T}","Max Cluster E","Hadronic Correction","Seed/Cell Energy","Tracking Efficiency","Trigger Swap p_{T}"});
        systematics_names.insert(systematics_names.end(), {"allunfolding","embedding","clusterizer","maxtrackpt","maxclustere","hadcorr","seedcell","tracking","triggerswap"});
        variation_original.insert(variation_original.end(), {"default","RandomCones","Clusterizerv2","200GeV","200GeV","F=1","300MeV","100%","(30GeV,60GeV)"});
        if(radius == 2 || radius == 3 || radius == 4) fitfunc.insert(fitfunc.end(), {"binwise","pol2","pol0","pol4","pol4","pol0","pol0","binwise","binwise"});
        else fitfunc.insert(fitfunc.end(), {"binwise","pol2","pol0","pol0","pol0","pol0","pol0","binwise","binwise"});
    //}else{
    ...
    for(int name=0; name<nSys; name++){
        systematicsFile = Form("/home/austin/alice/SpectrumPostProcessing/input_txt_files/pPbFiles/%s_%s.txt", systematics_names.at(name).Data(), type.Data());
    ...
    for(int name=0; name<nSys; name++){
        for(int i = 0; i < files[name].size(); i++){
            ...
            if(!regDirSys){
                cout << "Error: Directory R0" << radius << " not found!" << endl;
                return;
            }
            if(systematics_names[name] == "allunfolding" && files[name].at(i).Contains("SVD")){
    ...
    statError->GetYaxis()->SetRangeUser(-50,51);
